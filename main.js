...
const storyScreen = el('story-screen');
...
function showScreen(screenEl) {
  [mainMenu, wardrobeScreen, storyScreen, faqScreen, checkoutOverlay, resultsScreen].forEach((s) => s.classList.add('hidden'));
  if (screenEl) screenEl.classList.remove('hidden');
  isWardrobeOpen = screenEl === wardrobeScreen;
  if (isWardrobeOpen) {
    refreshWardrobePreview();
    resizeWardrobePreview();
  }
}
...
function renderWardrobe() {
  ...
  WARDROBE_SLOTS.forEach(({ key, label }) => {
    ...
    itemsInSlot.forEach((item) => {
      ...
      const swatch = `<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${hex(item.color)};margin-right:2px;vertical-align:middle;"></span>` +
        (item.accentColor ? `<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${hex(item.accentColor)};margin-right:5px;vertical-align:middle;"></span>` : '<span style="margin-right:5px;"></span>');
...
el('btn-story').addEventListener('click', () => {
  showScreen(storyScreen);
});

el('btn-story-back').addEventListener('click', () => {
  showScreen(mainMenu);
});
...
// ---------- Öltözőszekrény élő 3D előnézet ----------
const wardrobeCanvas = el('wardrobe-canvas');
const wardrobeRenderer = new THREE.WebGLRenderer({ canvas: wardrobeCanvas, antialias: true, alpha: true });
wardrobeRenderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
...
wardrobeScene.add(wardrobePreviewGroup);
let wardrobeCharacterMesh = null;
let wardrobeAutoRotate = true;

function refreshWardrobePreview() {
  if (wardrobeCharacterMesh) {
    wardrobePreviewGroup.remove(wardrobeCharacterMesh);
  }
  wardrobeCharacterMesh = buildPlayerCharacter(gameState.getOutfit());
  wardrobePreviewGroup.add(wardrobeCharacterMesh);
}

function resizeWardrobePreview() {
  const rect = wardrobeCanvas.getBoundingClientRect();
  if (rect.width === 0 || rect.height === 0) return;
  wardrobeRenderer.setSize(rect.width, rect.height, false);
  wardrobeCamera.aspect = rect.width / rect.height;
  wardrobeCamera.updateProjectionMatrix();
}
window.addEventListener('resize', () => {
  if (isWardrobeOpen) resizeWardrobePreview();
});

// --- Húzással forgatás (csak vízszintesen, ahogy kérted) ---
let wardrobeDragActive = false;
let wardrobeLastPointerX = 0;

function wardrobePointerDown(e) {
  wardrobeDragActive = true;
  wardrobeAutoRotate = false;
  wardrobeLastPointerX = e.clientX;
  wardrobeCanvas.setPointerCapture(e.pointerId);
}
function wardrobePointerMove(e) {
  if (!wardrobeDragActive) return;
  const delta = e.clientX - wardrobeLastPointerX;
  wardrobeLastPointerX = e.clientX;
  wardrobePreviewGroup.rotation.y += delta * 0.012;
}
function wardrobePointerUp(e) {
  wardrobeDragActive = false;
  try { wardrobeCanvas.releasePointerCapture(e.pointerId); } catch { /* no-op */ }
}
wardrobeCanvas.addEventListener('pointerdown', wardrobePointerDown);
wardrobeCanvas.addEventListener('pointermove', wardrobePointerMove);
wardrobeCanvas.addEventListener('pointerup', wardrobePointerUp);
wardrobeCanvas.addEventListener('pointercancel', wardrobePointerUp);

const wardrobeClock = new THREE.Clock();
function animateWardrobePreview() {
  requestAnimationFrame(animateWardrobePreview);
  if (!isWardrobeOpen) return;
  const delta = Math.min(wardrobeClock.getDelta(), 0.05);
  if (wardrobeAutoRotate) {
    wardrobePreviewGroup.rotation.y += delta * 0.4;
  }
